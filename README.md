...
- **多用户架构**：用户名密码注册 / 登录（JWT 鉴权，登录态 5 小时，bcrypt 密码哈希），MySQL 8.0 存储全部业务数据；知识库、会话历史、API 配置严格按用户隔离，互不可见；支持上传头像（默认用用户名首字符兜底）
...
- **按文档类型差异化分块**：操作指导 / BUG修复 / 表结构 / 其他四类分别配置 chunk_size 与 overlap；表结构类识别 Markdown 表格与 `CREATE TABLE` 特征，**以整张表为单位切分、字段名与注释不分离**，超大表按字段行拆分且每块重复表头（见下方「分块策略」）
...
- **版本更新提示**：后端维护当前版本号与 Markdown 更新日志，前端登录后比对本地记录，版本变化时弹窗（PC 用 Element Plus、移动端用 Vant 居中弹层），点「知道了」后不再重复弹出；管理员可在后台编辑发布
- **移动端自适应**：< 768px 自动切换移动布局——侧边栏纯 CSS 抽屉（汉堡按钮呼出 + 遮罩点击关闭）、对话气泡 85% 宽、输入区跟随软键盘（100dvh / visualViewport）不被遮挡、代码块/表格横向滚动、知识库三栏变单列并带悬浮上传菜单、确认框切 Vant Dialog、触控热区 ≥44px
- **会话管理**：侧边栏 DeepSeek 风格，按日期分组（置顶 → 今天 → 昨天 → 7 天内 → 更早），支持置顶 / 多选删除 / 实时搜索；会话与消息按用户隔离，越权访问返回 404
- **管理后台**：管理员（ancon 角色）可管理全部用户（禁用 / 启用 / 改角色 / 重置密码 / 删除，操作列主按钮 + 下拉菜单）、按条件筛选操作日志并导出 CSV、维护版本号与更新日志；首个注册用户自动成为管理员，也可在 `.env` 中通过 `ADMIN_USERNAME` 指定
...
入库时按文档 `doc_type` 从配置 `CHUNK_PROFILES` 选择切分参数，**参数集中在 `app/config/settings.py`，可在 `.env` 用 JSON 覆盖**：

| 类型 | chunk_size | overlap | 整段保留阈值 | 切分要点 |
| --- | --- | --- | --- | --- |
| schema 表结构 | 1000 | 50 | 4000 | 优先「一张表一个块」，字段名与注释同块；超大表按字段行分批，每块重复表头 |
| guide 操作指导 | 700 | 200 | 1000 | 按段落 / 步骤语义边界切，相邻块大重叠保证步骤上下文 |
| bug BUG修复 | 600 | 100 | 800 | 按案例 / 段落切 |
| other 其他 | 500 | 100 | 700 | 常规语义递归切分 |

补充规则：

- Markdown 表格（`| 字段 | 类型 | 注释 |` 分隔行）整体识别，绝不从一行字段中间断开；即使文档类型标注不准，内容含明显表格 / `CREATE TABLE` 特征时也按整表保护处理（[splitter.py](app/rag/splitter.py) 的 `looks_like_table_doc`）
- SQL / 命令行代码围栏（``` ）整体保留不拆散
- 每个 chunk 在 MySQL 与 Chroma 元数据中均带 `doc_type`，检索结果与给模型的上下文同步透出，便于调试
- **存量文档**：知识库管理页点「重建索引」即按各文档当前类型的新策略重新切分、重新向量化
...
| 前端（PC） | Vue 3 · Vite · TypeScript · Element Plus · Vue Router（登录/管理员守卫）· axios 拦截器（自动附 token，401 清态跳登录并提示） |
| 前端（移动） | Vant 4（弹层 / ActionSheet / Dialog）· postcss-px-to-viewport（vw 适配，设计稿 375px，**仅转换 Vant 样式，PC 像素零影响**）· 100dvh + visualViewport 软键盘适配 |
...
```
├── app/
│   ├── main.py           # FastAPI 入口：注册路由 + startup 初始化 MySQL/RAG/版本/会话存储
│   ├── rag/              # ① RAG 知识库（差异化分块 / 入库 / 混合检索 / 重排 / 表结构解析，Chroma 按用户隔离）
│   │   ├── splitter.py   # 语义分块器（章节/代码块/Markdown 表格识别，表结构整表保护）
│   │   ├── service.py    # CHUNK_PROFILES 按类型切分 + 重建索引 + 混合检索 + 相邻块合并
│   │   └── table_parser.py # DESC / DDL → 结构化表结构文本
...
│   ├── version/          # ⑤ 版本更新（GET/PUT /api/version，管理员维护版本号与更新日志）
│   ├── admin/            # 管理后台 API（用户 + 操作日志）
│   ├── auth/             # 用户认证（注册 / 登录 / me / 头像，JWT 解析，鉴权依赖）
│   ├── db/               # MySQL 引擎 / ORM 模型（含 operation_logs / app_version）
│   ├── core/             # LLM 客户端（按用户配置缓存）/ ReadOnlySQLGuard
│   └── config/           # 全局配置（settings.py，从 .env 读取，含 CHUNK_PROFILES）
...
│       ├── components/    # AppSidebar（移动端进纯 CSS 抽屉）/ SettingsDialog / VersionDialog
│       ├── api/           # request（鉴权实例 + 401 拦截）/ auth / chat / knowledge / admin / version
│       ├── store/         # 轻量全局状态（user / settings）
│       └── router/        # 路由 + 登录守卫 + 管理员守卫
├── scripts/              # 建库 / 多用户验证 / 检索测试 / 管理员验证 / 诊断脚本
├── tests/                # 单元测试
├── data/                 # Chroma 向量本地数据（自动生成；业务数据在 MySQL）
├── .env                  # 环境配置（MySQL / JWT / 百炼兜底 Key / CHUNK_PROFILES）
...
```bat
:: 创建 opsagent 库（utf8mb4；表结构由后端启动时自动创建 / 迁移 / 播种默认版本）
...
4. 管理员可在 `/admin` 页面管理用户、查看操作日志、发布版本更新
...
| 软键盘 | 布局高度用 `100dvh` + `--app-height`（visualViewport 监听写入），输入框在 flex 列底部自然贴底、始终位于键盘上方 |
| 侧边栏 | 纯 CSS 离屏抽屉（`transform: translateX`，宽 84vw / 最大 300px），顶栏汉堡按钮呼出，半透明遮罩点击关闭；PC 端为静态固定左栏、底部用户栏贴底 |
| 对话页 | 消息区 flex:1 + 内部滚动，用户气泡 max-width 85%；发送按钮 44px；代码块 / markdown 表格横向滚动；挂起 SQL 卡片纵向紧凑 |
| 知识库 | 三栏 grid 在移动端变单列且顺序为「上传 → 文档列表 → 检索测试」，整页自然文档流纵向滚动；右下角 56px 悬浮 + 号（仅移动端），点击弹 ActionSheet 选择上传方式 |
| 管理后台 | 表格包横滚容器（保持 640px 最小可读宽度，超宽横滚）；版本编辑页移动端单列堆叠 |
| 弹窗/确认 | 危险操作确认移动端走 Vant `showConfirmDialog`（红色确定按钮）；版本更新 / 设置等弹窗窄屏宽度统一约束 |
| 触控 | 按钮 / 输入框最小 44~48px 热区；输入框 16px 字号防 iOS 聚焦自动放大；去掉点击灰色高亮 |

## 多用户架构说明

### 数据表（MySQL `opsagent` 库，启动时自动建表 / 补列 / 播种）

| 表 | 说明 | 隔离方式 |
| --- | --- | --- |
| `users` | 用户（bcrypt 密码哈希、头像 data URL、状态、角色） | — |
| `user_configs` | 每用户模型配置（api_key / base_url / 各模型名） | `user_id` + `provider` 唯一 |
| `knowledge_docs` | 知识文档元数据（含 doc_type） | `user_id` 外键 |
| `knowledge_chunks` | 文档分块（含 doc_type / section） | 通过文档关联隔离 |
| `conversations` | 会话（状态机 / 挂起查询 / 轮次 / 置顶） | `user_id` 外键 |
| `messages` | 消息（user / assistant） | 通过会话间接隔离（查询前校验归属） |
| `operation_logs` | 操作审计日志（登录 / 对话 / 上传 / 删除 / 配置保存 / 版本发布等） | `user_id` 外键 |
| `app_version` | 当前版本号 + Markdown 更新日志（单行表 id=1，启动播种默认版本） | 全局共享 |
...
- **鉴权**：除注册 / 登录 / 健康检查外，所有接口需 `Authorization: Bearer <token>`；token 无效或过期返回 401，前端统一清态跳转 `/login` 并提示「登录已过期，请重新登录」（登录/注册接口本身的密码错误 401 不触发跳转；并发 401 只提示一次）
- **角色与权限**：`ancon`（管理员）可见 `/admin` 后台入口，能管理所有用户、查看全部操作日志、导出 CSV、发布版本；`user`（普通用户）看不到后台入口，访问 `/admin` 会被自动跳回首页
...
- 后端 `app_version` 单行表存 `version`（如 v1.1）与 `changelog`（Markdown），`GET /api/version` 任意登录用户可读，`PUT /api/version` 仅管理员可改并写审计日志
- 前端登录会话拉取版本，与 `localStorage.opsagent_last_version` 比对，不一致（含首次登录）弹更新弹窗；「知道了」写入本地版本后不再弹，「查看详情」展开完整日志
- 管理员在 `/admin` →「版本更新」编辑版本号 + Markdown 日志（带实时预览），保存发布后所有用户下次登录 / 刷新重新看到提示
...
:: 单元测试（含分块器差异化 / 表结构整表保护用例）
.venv\Scripts\python.exe -m pytest tests/ -q
```

登录过期自测：`.env` 设 `JWT_EXPIRE_MINUTES=1` 重启后端 → 登录 → 等 1 分钟后任意操作应自动跳登录页并提示；验证完改回 `0`。
...
| `CHUNK_PROFILES` | 按 doc_type 的分块参数（JSON：schema/guide/bug/other 各自 chunk_size / chunk_overlap / whole_threshold） | 见 settings.py |
...
| 频繁提示登录过期 / 跳登录页 | token 默认 5 小时有效，过期需重新登录，属预期安全策略；自测可临时设 `JWT_EXPIRE_MINUTES`；后端重启不清未过期 token（JWT_SECRET 未变时） |
...
| 知识检索不准 / 字段被切断 | 确认文档类型标注正确（表结构类整表保护）；调整分块参数后到知识库页点「重建索引」对存量文档重新切分；文档按账号隔离，先确认登录的是上传文档的账号 |
| 改了文档类型没变化 | 「改类型」只更新分类元数据；若需按新类型重新切分，请点「重建索引」 |
...
3. **登录态安全**：token 有效期 5 小时（`JWT_EXPIRE_HOURS`），过期任意接口返回 401 即强制重新登录，不做长期免登录；前端不长期留存凭证
4. **账号与数据安全**：密码仅存 bcrypt 哈希；业务数据在 MySQL 按用户隔离，向量数据在 Chroma 按用户 collection 隔离；接口越权访问他人会话返回 404；删除用户时级联清理其全部数据
5. **API-KEY 安全**：每个用户的 Key 只存自己的 `user_configs` 行；接口仅返回掩码 + `has_api_key` 标志，不回显明文；日志不打印密钥；如怀疑泄露请立即在百炼控制台吊销重建
6. **配置安全**：`.env` 含 MySQL 密码与 JWT 密钥，**切勿提交代码仓库**；生产环境务必更换 `JWT_SECRET`
7. **审计留痕**：关键操作（登录 / 注册 / 对话 / 上传 / 删除 / 检索 / 配置保存 / 用户管理 / 版本发布）均记录到 `operation_logs`，含用户名、操作类型、详情摘要、IP 与时间，管理员可在后台按条件筛选导出
